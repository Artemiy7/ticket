...
@Component
public class ConcertClubInitialLoader implements InitialLoader<OccasionEntity> {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(ConcertClubInitialLoader.class);
    private final String filePath = OccasionLoader.CONCERT_CLUB_FILE.getFilePath();

    public ConcertClubInitialLoader(OccasionJsonFileUtil occasionJsonFileUtil,
                ...
                occasionLoaderDtoList.forEach(occasionLoaderDto -> {
                    if (LocalDateTime.now().isAfter(occasionLoaderDto.getOccasionTime().plusMinutes(30))) {
                        LOGGER.info(occasionLoaderDto.getOccasionName() + " " + occasionLoaderDto.getOccasionAddress()
                                ...
                                + " is outdated");
                        return;
                    } else if (!occasionLoaderDto.getTicketType().equals(TicketType.CONCERT_CLUB)) {
                        LOGGER.info(occasionLoaderDto.getOccasionName() + " " + occasionLoaderDto.getOccasionAddress()
                                + " " + occasionLoaderDto.getOccasionTime() + " " + occasionLoaderDto.getTicketType()
                                + " TicketType is wrong");
                        return;
