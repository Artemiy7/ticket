...
@Component
public class ConcertStadionInitialLoader implements InitialLoader<OccasionEntity> {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(ConcertStadionInitialLoader.class);
    private final String filePath = OccasionLoader.CONCERT_CLUB_STADION_FILE.getFilePath();

    public ConcertStadionInitialLoader(OccasionJsonFileUtil occasionJsonFileUtil,
            ...
            if (file.exists()) {
                ...
                occasionLoaderDtoList.forEach(occasionLoaderDto -> {
                    if (LocalDateTime.now().isAfter(occasionLoaderDto.getOccasionTime().plusMinutes(30))) {
                        LOGGER.error(occasionLoaderDto.getOccasionName() + " " + occasionLoaderDto.getOccasionAddress()
                                + " " + occasionLoaderDto.getOccasionTime() + " " + occasionLoaderDto.getTicketType()
                                + " is outdated");
                        return;
                    } else if (!occasionLoaderDto.getTicketType().equals(TicketType.CONCERT_STADION)) {
